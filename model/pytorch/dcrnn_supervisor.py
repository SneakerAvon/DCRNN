...
from model.pytorch.loss import masked_rmse_loss
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                mape_loss = self._compute_mape_loss(y, output)
                rmse_loss = self._compute_rmse_loss(y, output)
            ...
            mean_mape_loss = np.mean(mape_losses)
            mean_rmse_loss = np.mean(rmse_losses)
            ...
            return mean_loss, mean_mape_loss, mean_rmse_loss, {'prediction': y_preds_scaled, 'truth': y_truths_scaled}
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                #self._logger.debug(mape_loss.item())
                #self._logger.debug(rmse_loss.item())
            ...
            val_loss, val_mape_loss, val_rmse_loss, _ = self.evaluate(dataset='val', batches_seen=batches_seen)
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, val_mae: {:.4f}, val_mape: {:.4f}, val_rmse: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), val_loss, val_mape_loss, val_rmse_loss, lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)

            if (epoch_num % test_every_n_epochs) == test_every_n_epochs - 1:
                test_loss, test_mape_loss, test_rmse_loss, _ = self.evaluate(dataset='test', batches_seen=batches_seen)
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, test_mae: {:.4f},  test_mape: {:.4f},  test_rmse: {:.4f},  lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), test_loss, test_mape_loss, test_rmse_loss, lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)
